...
INSUMSTATS=./Sumstats/NonCog_GWAS_110320.txt
OUTLDPRED=./NONCOG_11032020_ldpred_in.dat

cat $INSUMSTATS | sed '1,1d' | awk '{print $2" "$3" "$4" "$5" 0.50 "$7" "$8" "$10" 458211"}' > _Sum1.dat
...
SUMST=$OUTLDPRED
OUT=NONCOG_CEU.coord
GT=./LDPred/EUROPEAN_1KG_P3V5A_B37_FixJJ_C1-22_LDPred
echo $SUMST 
cut -d' ' -f3 $SUMST | sed '1,1d' > _Needed_snps.dat
plink190 --bfile $GT --extract _Needed_snps.dat --maf 0.01 --make-bed --out _GTdata
ldpred coord --gf=_GTdata --ssf-format=LDPRED --max-freq-discrep=0.50 --ssf=$SUMST --out=$OUT > NONCOG_coord.log
rm _*
